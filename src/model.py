...
class Wav2Vec2Module(torch.nn.Module):
    def __init__(
        ...
        self.feature_extractor = AutoFeatureExtractor.from_pretrained(self.wav2vec2_model_name)
...
###########################################################################
# WavLM - Couldn't figure it out :(. It just wasn't training, and was stuck at ~50% accuracy and seemingly moving up and down at random.
###########################################################################
...
class HubertModule(torch.nn.Module):
    def __init__(
        ...
        self.feature_extractor = AutoFeatureExtractor.from_pretrained(self.hubert_model_name)
...
###########################################################################
# Parakeet - the code probably works but the transformer library in the singularity container is too old and doesn't contain the parakeet model :(
###########################################################################
